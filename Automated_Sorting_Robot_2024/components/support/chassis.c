...
/*底盘移动控制*/
void chassis_control(float x, float y, float z, uint8_t move_mode, chassis_move_t *chassis_move_control)
{
	if(move_mode == SPEED_MODE)
	{
		chassis_move_control->vx_set = PID_calc(&chassis_move_control->chassis_speed_pid_x, chassis_move_control->vx, x);
		chassis_move_control->vy_set = PID_calc(&chassis_move_control->chassis_speed_pid_y, chassis_move_control->vy, y);
		chassis_move_control->wz_set = PID_calc(&chassis_move_control->chassis_speed_pid_yaw, chassis_move_control->yaw_speed, z);
	}
	if(move_mode == LOCATION_MODE)
	{
		//加入斜坡函数实现缓起
		ramp_function_chassis(data_follow, x, ramp_k);
		ramp_function_chassis(data_follow + 1, y, ramp_k);
		ramp_function_chassis(data_follow + 1 + 1, z, ramp_k*1000);
...
void chassis_init(chassis_move_t *chassis_move_init)
{
	...
	const static float chassis_yaw_speed_pid[3] = { CHASSIS_YAW_SPEED_PID_KP, CHASSIS_YAW_SPEED_PID_KI, CHASSIS_YAW_SPEED_PID_KD };
	...
	PID_init(&chassis_move_init->chassis_speed_pid_yaw, PID_POSITION, chassis_yaw_speed_pid, CHASSIS_YAW_SPEED_PID_MAX_OUT ,CHASSIS_YAW_SPEED_PID_MAX_IOUT);
...
/*斜坡函数*/
void ramp_function_chassis(float *data_follow, float data_target, float k)//k：步进的幅度
{
...
/*里程计清零*/
void chassis_odometry_reset(chassis_move_t *chassis_move_reset)
{
		odometryResetFlag = 1;
