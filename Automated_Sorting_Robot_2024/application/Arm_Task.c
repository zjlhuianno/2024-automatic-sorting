...
//		force_sensing_flag = 1;//启动力控感知。
		...
		if (look_frame_cnt == 0)//机械臂伸到高缓冲处，爪闭合。
		{
			set_arm_pos_param(38.0f, 30.0f, 30.0f, 5.0f);//高缓冲处。
			look_stable_function(0, 0);//不是最后一帧，且爪闭合。
		}
		else if (look_frame_cnt == 1)//机械臂仍保持高缓冲处，爪闭合，进行openmv识别。
		{
			set_arm_pos_param(38.0f, 30.0f, 30.0f, 5.0f);//高缓冲处。
//			mode_openmv = 1;//打开openmv的识别形状颜色模式。(不识别二维码。)
			...
			if (color == 1)//如果颜色是红色。
			{
				...
				{
					look_frame_cnt = 2;//openmv看的动作结束。
					catch_object(1);//低平台夹方块。
				}
				else if (shape == 1)//如果形状为圆环。
				{
					look_frame_cnt = 2;//openmv看的动作结束。
					catch_object(2);//低平台夹甜甜圈。
				}
			}
		}		
	}//mode == 1结束。
