...
//		force_sensing_flag = 1;//启动力控感知。
		...
		{
			set_arm_pos_param_true(38.0f, 22.0f, 0.0f);//圆盘机平台。
			pos_stable_function(0, 0);//不是最后一帧，且爪闭合。
		}
...
//		force_sensing_flag = 1;//启动力控感知。
		...
		else if (pos_frame_cnt == 1)//机械臂仍保持低平台看处，爪闭合，进行openmv识别。
		{
			set_arm_pos_param_true(38.0f, 25.0f, 0.0f);//低平台看。
...
//		force_sensing_flag = 1;//启动力控感知。
		...
		else if (pos_frame_cnt == 1)//机械臂仍保持高平台看处，爪闭合，进行openmv识别。
		{
			set_arm_pos_param_true(40.0f, 25.0f, 5.0f);//高平台看。
//			mode_openmv = 1;//打开openmv的识别形状颜色模式。(不识别二维码。)
			//main_task中会提前把mode_openmv置1，即打开openmv的识别形状颜色模式。
			if (color == 1)//如果颜色是红色。
			{
...
void catch_object(uint8_t mode)
{
	...
	if (mode == 1)//低平台夹方块。
	{
		...
		if (pos_frame_cnt == 0)//机械臂伸到低平台上方，爪闭合。
		{
			...
			set_arm_pos_param_true(38.0f, 25.0f, 0.0f);//低平台上方。
			pos_stable_function(0, 1);//不是最后一帧，且爪张开。
			
		}
		else if (pos_frame_cnt == 1)//机械臂伸到低平台，爪取方块。
		{
			set_arm_pos_param_true(38.0f, 12.0f, 0.0f);//低平台。
		...
		else if (pos_frame_cnt==2)//机械臂伸到低平台的较高处，爪仍保持抓方块的姿态，为的是防止爪放方块时方块会搓到阶梯平台。
		{
			...
			set_arm_pos_param_true(38.0f, 20.0f, 0.0f);//低平台的较高处。
			pos_stable_function(0, 2);//不是最后一帧，且爪取方块。
...
void FUCK_YOU(void)
{
	...
	if(catch_object_mode == 7)//立桩抓球。
	{
		catch_object(7);
	}
	if(catch_object_mode == 8)//把甜甜圈从机器桩夹到木桩。
	{
		catch_object(8);
	}	
