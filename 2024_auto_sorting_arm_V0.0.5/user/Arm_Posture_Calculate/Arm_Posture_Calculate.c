...
//	Z_P = Target_Point[1];
//	
//	//解算后得出的量，依序存储到Angle[3]中。
//	float Alpha = 0.0f;	//α = ∠OAB ，由DM4340  控制。
//	float Beta  = 0.0f;	//β = ∠ABC ，由DM4310  控制。
//	float Gamma = 0.0f;	//γ = ∠B'CD，由关节舵机控制。（B'为射线BC上的一点。）
//	
//	//计算Beta。
//	float Cos_Beta;//Beta的余弦值。
//	float A[2], C[2];//A,C两点的坐标。
//	
//	A[0] = 0.0f;
//	A[1] = L0;
//	C[0] = Y_P - L3;
...
//	
//	//计算Alpha。
//	float Cos_Alpha;//Alpha的余弦值。
//	float a, b, c, m, n;//存储参数的临时变量。
//	
//	a = L2 / L1;
//	b = (Y_P - L3) / L1;
//	c = (L0 - L4 - Z_P) / L1;
